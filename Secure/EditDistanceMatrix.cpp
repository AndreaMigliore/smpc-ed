...
int edit_distance(vector<int> sequence, int lengthAlice, int lengthBob, int intBitSize) {
    ...
    for (x = 1; x <= lengthBob; x++)
        matrix[x][0] = matrix[x-1][0] + one;
    for (y = 1; y <= lengthAlice; y++)
        matrix[0][y] = matrix[0][y-1] + one;

    for (x = 1; x <= lengthBob; x++)
        for (y = 1; y <= lengthAlice; y++)
            matrix[x][y] = minimum(matrix[x-1][y] + one, matrix[x][y-1] + one, matrix[x-1][y-1] + one.select((sequenceAlice[y-1] == sequenceBob[x-1]), zero));

    return matrix[lengthBob][lengthAlice].reveal<int>();
}
